...
**Cause probable** : Le microservice démarre avant que ses dépendances (MySQL, ConfigServer, Eureka) ne soient prêtes.

**Solution** : Patientez 2 à 3 minutes. Le mécanisme de restart de Kubernetes va automatiquement relancer le pod. Vérifiez les logs pour confirmer :
```bash
kubectl logs <nom-du-pod> -n miage-bank
```
...
**Cause probable** : Les images Docker ne sont pas disponibles dans l'environnement Kubernetes.

**Solutions selon l'environnement** :
- **Minikube** : Assurez-vous d'avoir exécuté `eval $(minikube docker-env)` **avant** le build des images
- **Docker Desktop** : Les images buildées localement sont automatiquement disponibles
- **Cluster distant** : Poussez les images vers un registre (ex: `ghcr.io`) et mettez à jour les `image.repository` dans `values.yaml`

---

### `port-forward` → `address already in use`

**Solution** : Utilisez un port local différent :
```bash
kubectl port-forward svc/bnkapigateway 10002:10000 -n miage-bank
#                                       ^^^^^ port local modifié
```
...
### ArgoCD affiche `Unknown` ou ne synchronise pas

**Causes possibles** :
1. Le dépôt GitHub n'est pas accessible → vérifiez l'URL dans `argocd-app.yaml`
2. Les derniers changements ne sont pas poussés sur `main` → faites un `git push`
3. Les CRDs d'ESO ne sont pas installées → vérifiez avec `kubectl get crd | grep external`

---

### Vault : `connection refused` ou `permission denied`

**Solutions** :
- Vérifiez que le pod Vault est en cours d'exécution :
  ```bash
  kubectl get pods -n default | grep vault
  ```
- Vérifiez que le secret `vault-token` existe dans le namespace `miage-bank` :
  ```bash
  kubectl get secret vault-token -n miage-bank
  ```
- En mode dev, le token root est `root` — vérifiez qu'il correspond

---

### Helm lint renvoie des warnings

**Solution** : Les warnings ne sont pas bloquants, mais les erreurs doivent être corrigées avant le déploiement. Les warnings les plus courants concernent des clés non utilisées dans `values.yaml` et sont sans danger.

---

### Architecture ARM (Apple Silicon M1/M2/M3)

Les images construites en CI sont en `amd64`. Si vous déployez sur un Mac ARM :

1. Exécutez `eval $(minikube docker-env)` puis `./build-all-images.sh` pour rebuilder en natif sur votre architecture
2. Ou lancez Minikube avec le driver Docker (qui inclut l'émulation) : `minikube start --driver=docker`
